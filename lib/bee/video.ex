defmodule Bee.Video do
  ...
  def init (opts) do
    ...
    case :gen_udp.open(opts.port, [:binary, active: true, ip: opts.local_ip]) do
      { :ok, port } ->
        :gen_udp.send(port, opts.remote_ip, opts.port, Bee.Packet.agent_video_ack )
        {:ok, %{port: port, connection_port: opts.port, remote_ip: opts.remote_ip, sequence: 0, video: %Bee.Video{}} }
      { :error, :eaddrnotavail } ->
        :timer.sleep(1000)
        init(opts)
    end
